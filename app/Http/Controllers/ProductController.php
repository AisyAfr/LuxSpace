    ...
    public function index()
    {
        if(request()->ajax()){
            ...
            return DataTables::of($query)->addColumn('action', function($item){
                return '<a href = "'.route('dashboard.product.gallery.index', $item->id). '" class = "bg-emerald-300 hover:bg-white hover:text-emerald-400 text-white font-bold py-2 px-4 row shadow-lg rounded mx-2">Gallery</a>
                <form class = "inline-block" method = "POST" action = "'.route('dashboard.product.destroy', $item).'">
                <a href = "'.route('dashboard.product.edit', $item->id). '" class = "bg-pink-400 hover:bg-white hover:text-pink-500 text-white font-bold py-2 px-4 row shadow-lg rounded " >Edit</a>
                <form class = "inline-block" method = "POST" action = "'.route('dashboard.product.destroy', $item).'">
